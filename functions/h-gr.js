//!--------------------------- SCRAPER HOMBRES GALPON DE ROPA ---------------------------
...
async function scrapeAndWriteToGoogleSheet(numeroMaximoOprimidos = 5) {
  ...
  try {
    ...
    const spreadsheetId = '15EQmRVaDxjQvwvMnIawAvjeRNDyy--GhTo_x5Q8-bGQ';

    // Navega a una página web
    await page.get('https://www.galponderopa.com/191?map=productClusterIds');
    ...
    for (const element of superElements) {
      try {
        ...
        dataByCategory[category].push([name, priceNumber, link, marcaName]);
      } catch (error) {
        console.log(error);
        continue;
      }
